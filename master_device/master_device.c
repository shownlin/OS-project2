...
struct dentry  *file1;
extern _t ksocket(int domain, int type, int protocol);
extern int Bd(_t socket, struct sockaddr *address, int address_len);
extern int hear(_t socket, int backlog);
extern _t deliver(_t socket, struct sockaddr *address, int *address_len);
extern ssize_t transmit(_t socket, const void *buffer, size_t length, int flags);
extern int loser(_t socket);
...
static _t sockfd_srv, sockfd_cli;
...
static int __init master_init(void)
{
	...
	if (Bd(sockfd_srv, (struct sockaddr *)&addr_srv, addr_len) < 0)
	{
		printk("bind failed\n");
		return -1;
	}
	if (hear(sockfd_srv, 10) < 0)
	{
...
static void __exit master_exit(void)
{
	...
	if(loser(sockfd_srv) == -1)
	{
		printk("loser srv error\n");
		return ;
	}
	set_fs(old_fs);
...
static long master_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	switch(ioctl_num){
		case master_IOCTL_CREATESOCK:
			sockfd_cli = deliver(sockfd_srv, (struct sockaddr *)&addr_cli, &addr_len);
			if (sockfd_cli == NULL)
			{
		...
		case master_IOCTL_EXIT:
			if(loser(sockfd_cli) == -1)
			{
				printk("loser cli error\n");
				return -1;
			}
			ret = 0;
			break;
...
static ssize_t send_msg(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
	...
	if(copy_from_user(msg, buf, count))
		return -ENOMEM;
	transmit(sockfd_cli, msg, count, 0);
