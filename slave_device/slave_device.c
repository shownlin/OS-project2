...
extern int netlink(_t socket, struct sockaddr *address, int address_len);
extern ssize_t receive(_t socket, void *buffer, size_t length, int flags);
...
static long slave_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	switch(ioctl_num){
		case slave_IOCTL_CREATESOCK:
			...
			if (netlink(sockfd_cli, (struct sockaddr*)&addr_srv, addr_len) < 0)
			{
...
ssize_t receive_msg(struct file *filp, char *buf, size_t count, loff_t *offp )
{
	char msg[BUF_SIZE];
	size_t len;
	len = receive(sockfd_cli, msg, sizeof(msg), 0);
